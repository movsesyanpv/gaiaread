...
contains
	...
	Subroutine LSQM(a,y,w, x,d, s, r, cond)
...
! cond  Число обусловленности
	...
	if (size(y)/=m .or. size(w)/=m .or. size(x)/=n .or. size(d)/=n .or. size(r,dim=1)/=n .or. size(r,dim=2) /= n) then
	 STOP 'Incorrect parameters of LSQM'
      ...
      !do i=1,n
	  ! print "(20F8.2)",(R(i,j),J=1,N)
	  !end do

      cond=Cond1(r) ! Число обусловленности
      call Invert(r)
      call Multiply(r,c,x)
...
REAL(8) FUNCTION MATRIXNORM1(D)
	real(8), intent(in) :: D(:,:)		! Исходная матрица	
    INTEGER(4) :: N
	INTEGER(4) :: I,J
	REAL(8) :: S
	N = size(D,dim=1)
	...
	DO J=1,N
	 ...
	 DO I=1,N
	  S=S+DABS(D(I,J))
	 END DO
	 IF (S>MATRIXNORM1) MATRIXNORM1=S
	END DO
END FUNCTION


REAL(8) FUNCTION COND1(D)
 	real(8), intent(in) :: D(:,:)		
	real(8) :: R(size(D,dim=1),size(D,dim=1))		
    R=D
	CALL INVERT(R)
	COND1=MATRIXNORM1(D)*MATRIXNORM1(R)
END FUNCTION
